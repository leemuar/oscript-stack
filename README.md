...
![Анимация получения элементов из стека](docs/pop-animation.svg)
...
Рекомендуемый способ - с помощью менеджера пакетов [opm](https://github.com/oscript-library/opm):

```
opm install collection-stack
```
После установки библиотеки подключите ее в файле скрипта

```bsl
#Использовать collection-stack
```

После этого в системе типов скрипта станет доступен тип `Стек`, можно будет создавать объекты: `МойСтек = Новый Стек;`.

Без opm — подключите файл `src/stack.os` в своём проекте как сценарий:
